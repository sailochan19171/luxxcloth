...
import VirtualTryOnWrapper from "./VirtualTryOnWraaper";
...
interface ProductDetailProps {
  ...
  onRequestLogin?: () => void;
  onBack?: () => void;
  onBuyNow?: (product: Product, color: Color, size?: Size, quantity?: number) => void;
}

const ProductDetail: React.FC<ProductDetailProps> = ({ product, onRequestLogin, onBack, onBuyNow }) => {
  const { dispatch } = useCart();
  const [selectedColor, setSelectedColor] = useState(product.colors[0]);
  const [selectedSize, setSelectedSize] = useState<Size | null>(product.sizes?.[0] || null); // Initialize with first size if available
  ...
  const handleAddToCart = () => {
    if (!selectedColor || (product.sizes?.length && !selectedSize)) {
  ...
  const handleBuyNow = () => {
    // Only require selectedSize if product.sizes exists and is non-empty
    if (!selectedColor || (product.sizes?.length && !selectedSize)) {
      alert("Please select all required options");
      return;
    }
    ...
    if (onBuyNow) {
      onBuyNow(product, selectedColor, selectedSize || undefined, quantity);
    } else {
